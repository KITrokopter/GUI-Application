	...
	cameraSystem(camsys)
{
	...
	// Connect the tab change signal before setting up the tabs to capture the
...
void CalibrationDialog::takePictureDone()
{
	...
	for (auto &cs : status) {
		...
		statusText.append(QString("Camera %1: %2 (%3)<br>")
		                  .arg(cs.first)
		                  .arg(cs.second ? QString::fromUtf8("<span style='color: green'>\u2713</span>") : QString::
		                       fromUtf8("<span style='color: red'>\u2717</span>"))
