...
GUIQuadcopter::GUIQuadcopter(ISceneNode *parent, std::vector<double> positionVector, std::vector<double> rotationMatrix,
...
void GUIQuadcopter::createMesh(ISceneManager *sceneManager)
{
	...
	mesh =
	    sceneManager->addArrowMesh("Arrow", SColor(255, 255, 0, 255), SColor(255, 255, 0, 255), 4, 8, 160, 140, 4, 12);
	node = sceneManager->addAnimatedMeshSceneNode(mesh, getSceneNode(), -1, vector3df(-75, 0, 0));
	node->setRotation(vector3df(90, 90, 0));
	// addShadowVolumeSceneNode(node->addShadowVolumeSceneNode());

	mesh = sceneManager->addArrowMesh("Cylinder", SColor(255, 255, 0, 255), SColor(255, 255, 0,
	                                                                               255), 4, 8, 150, 149.999f, 4, 4);
