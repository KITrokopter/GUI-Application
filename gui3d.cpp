...
void Gui3D::initializeIrrlicht()
{
	...
	cameraNode =
	    sceneManager->addCameraSceneNode(cameraCenterNode, vector3df(0, 0, cameraDistance), vector3df(0, 0, -1));
	...
	ISceneNode *arrowNode =
	    sceneManager->addMeshSceneNode(sceneManager->addArrowMesh("x-axis",
	                                                              SColor(255, 255, 0,
	                                                                     0),
	                                                              SColor(255, 255, 0,
                                                                         0), 4, 8, 150, 140, 8, 8), simulationNode);
	arrowNode->setRotation(vector3df(90, 90, 0));
	arrowNode->setMaterialFlag(EMF_LIGHTING, false);
	arrowNode =
	    sceneManager->addMeshSceneNode(sceneManager->addArrowMesh("y-axis",
	                                                              SColor(255, 0, 255,
	                                                                     0),
	                                                              SColor(255, 0, 255,
                                                                         0), 4, 8, 150, 140, 8, 8), simulationNode);
	...
	arrowNode->setMaterialFlag(EMF_LIGHTING, false);
	arrowNode =
	    sceneManager->addMeshSceneNode(sceneManager->addArrowMesh("z-axis",
	                                                              SColor(255, 0, 0,
	                                                                     255),
	                                                              SColor(255, 0, 0,
                                                                         255), 4, 8, 150, 140, 8, 8), simulationNode);
	...
	c0 = new GUICamera(simulationNode, "install/lib/gui_application/models/camera.3ds", vector3df(0, 500,
	                                                                                              0), sceneManager);
	#endif
...
void Gui3D::updateQuadcopters()
{
	...
	for (std::vector<kitrokopter::APIQuadcopter*>::iterator it = quadcopters.begin(); it != quadcopters.end(); it++) {
		if (this->quadcopters.count((*it)->getId()) == 0) {
			this->quadcopters[(*it)->getId()] =
			    new GUIQuadcopter(simulationNode, std::vector<double>(3), std::vector<double>(9),
			                      iw->getSceneManager());
		}
...
void Gui3D::updateCameras()
{
	...
	for (auto camera : cameras) {
		if (this->cameras.count(camera->getId()) == 0) {
			this->cameras[camera->getId()] = new GUICamera(simulationNode,  ":/3d/models/camera.3ds", std::vector<double>(
			                                                   3), std::vector<double>(9), iw->getSceneManager());
		}
